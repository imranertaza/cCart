...
<div class="main-container margin-t-b" id="tableReload2">
    <div class="container">
        <div class="cart">
            <div class="row">
                <div class="col-md-12 ">
                    <?php //f (session()->getFlashdata('message') !== null) : echo session()->getFlashdata('message');endif;
                    $modules = modules_access();
                    ?>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table table-bordered cart-table w-100 text-center " >
                    <thead>
                        <tr>
                            <th>Delete</th>
                    ...
                    foreach (Cart()->contents() as $val) { ?>
                            <tr>
                                <td class="product-remove mo-text-center">
                                    <a href="javascript:void(0)" onclick="removeCart('<?php echo $val['rowid']; ?>')"><i class="fa-solid fa-trash-can text-danger"></i></a>
                                </td>
                                ...
                                <td class="product-name text-start mo-text-center">
                                    <a href="<?= base_url('detail/' . $val['id'])?>" class="text-black"><?php echo $val['name']; ?></a>
                                </td>
                                ...
                                <td class="product-quantity mo-text-center" width="180">
                                    <div class="quantity d-flex justify-content-end justify-content-lg-center">
                                        <div class="input-group mb-3">
                                            ...
                                            <input type="text" id="qty_input" name="qty" class="form-control text-center form-control-sm item_<?php echo $val['rowid']; ?>" value="<?php echo $val['qty']; ?>" min="1">
            ...
            <?php if (!empty(Cart()->contents())) { ?>
                <p class="text-end"><a href="<?php echo base_url('checkout') ?>" class="btn bg-custom-color text-white rounded-0 px-4 btn-checkout-2">Proceed to checkout</a></p>
            <?php } ?>
        </div>
    </div>
</div>
