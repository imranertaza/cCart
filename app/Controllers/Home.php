...
class Home extends BaseController {
    ...
    public function verify(){
        ...
        if (($email_d == $this->session->email) && ($otp_d == $this->session->otp)){
            ...
            $this->session->setFlashdata('message', '<div class="alert-success_web py-2 px-3 border-0 text-white fs-5 text-capitalize" role="alert">Something went wrong please try again! </div>');
            return redirect()->to('/');
        }
